...
//    Optional<VolunteerInfo> findByVolTime(Float volTime);
    ...
    //사용자가 완료한 봉사 내역 리스트 봉사 완료 시간 순으로 확인
    ...
    //사용자가 신청한 봉사내역 확인(완료 내역 포함)
    @Query("Select m from VolunteerInfo m where m.whoVol = :whoVol order by m.whenapplied")
    List<VolunteerInfo> findByWhoVolOrderByWhenapplied(@Param("whoVol") Long whoVol);
